...
def joint_loss(labels, class_preds, trans_targets, trans_preds, cell_number_targets, cell_number_preds, peak_targets, peak_preds, boost_mask, balanced=False):
    ...
    #     prob_weight = 1 - np.tile(pred_points[:,-1], ( label_x_detected.shape[0], 1))#the higher the prob, the shorter the distance
    #
    #     print(prob_weight.shape,pred_points[:,-1].shape)
    ...
    #hidden_preds = peak_preds <= 0.5
    #peak_preds[hidden_preds] = peak_targets[hidden_preds]
    #print(peak_preds,peak_targets)
    #hungarian_loss = F.binary_cross_entropy(peak_preds, assignment_map)
    neg_length = peak_preds[peak_preds>0.5].size()[0]
    pos_length = peak_targets[peak_targets==1].size()[0]
    neg_weight = torch.ones(neg_length)*(neg_length+pos_length)/neg_length
    pos_weight = torch.ones(pos_length)*(neg_length+pos_length)/pos_length
    peak_weight = torch.cat((neg_weight,pos_weight))
    if use_gpu:
        peak_weight = Variable(peak_weight).cuda()


    hungarian_loss = F.binary_cross_entropy(torch.cat((peak_preds[peak_preds>0.5],peak_preds[peak_targets==1])), torch.cat((peak_targets[peak_preds>0.5],peak_targets[peak_targets==1])), weight=peak_weight)
    #print(peak_preds[peak_preds>0.5].size())
    #hungarian_loss = F.binary_cross_entropy(peak_preds[peak_targets==1],peak_targets[peak_targets==1])
    if num_pos > 0:
...
def train_model(model, optimizer, scheduler, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for sample in dataloders[phase]:
                ...
                running_cls_loss += cls_loss.item()*8
                running_peak_loss += peak_loss.item()*8
                if phase == 'train':
            ...
            epoch_peak_loss = running_peak_loss / dataset_sizes[phase]
            epoch_acc = (running_corrects / running_sizes).mean()
            epoch_loss = epoch_peak_loss
            print('{} Cls_Loss: {:.4f} Reg_Loss: {:.4f} Num_Loss: {:.4f} Acc: {:.4f} Peak_Loss: {:.4f}'.format(
                phase, epoch_cls_loss, epoch_reg_loss, epoch_num_loss, epoch_acc, epoch_peak_loss))
    ...
    print('Best val Peak_Loss: {:4f}'.format(best_loss))
...
if option == 'train':
    ...
    pretrained_dict = torch.load(model_directory+'trained_cell_vgg11_jointweighted_vgg_%s_backup.pth' % dis_thresh)
    ...
    model_ft = train_model(model_ft, optimizer_ft, exp_lr_scheduler,
                           num_epochs=10)
    torch.save(model_ft.state_dict(), model_directory+'trained_cell_vgg11_joint%s_%s_%s_doublefcn.pth' % (loss_option, config, dis_thresh))
