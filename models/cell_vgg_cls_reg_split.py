...
class cell_vgg_cls_reg_split(nn.Module):

    def __init__(self, vgg_name, subnet_layer_number=2):
        super(cell_vgg_cls_reg_split, self).__init__()
        self.encoder = self._make_encoder(cfg[vgg_name])
        self.decoder = self._make_decoder(cfg[vgg_name])
    ...
    def forward(self, x):
        ...
        cls_map = self.decoder(x)
        cls_map = self.classifier(cls_map)

        reg_map = self.decoder(x)
        reg_map = self.regressor(reg_map)

        qtf_map = self.decoder(x)
