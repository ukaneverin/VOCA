...
parser.add_argument('--r', default=12, type=int, help='the hyperparameter r for disk size')
...
for file in glob.glob(annotation_path+'*.json'):
    if os.stat(file).st_size > 10: #check good json files
...
#get the cross valication split for image names and patch librarys
...
#generate the task maps for VOCA (saving this to disk as a trade off between disk memory and training speed)
for image_name in image_list:
    ...
    #task_map: prediction target for VOCA; channels: [cls_map, x_map, y_map, wt_map]
    ...
    for p in coords_data:
        ...
        task_map[update_mask, args.num_classes+c] = x_map[update_mask] # update x_map
        task_map[update_mask, 2*args.num_classes + c] = y_map[update_mask] # update y_map
